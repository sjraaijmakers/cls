...
int main(int argc, char *argv[]) {
	...
	person_map   = (Person_new *)         mmapr(makepath(argv[1], "person_reorg_2",   "bin"), &person_length);
	interest_map = (unsigned short *) mmapr(makepath(argv[1], "interest", "bin"), &interest_length);
	knows_map    = (unsigned int *)   mmapr(makepath(argv[1], "knows_reorg_2",    "bin"), &knows_length);
