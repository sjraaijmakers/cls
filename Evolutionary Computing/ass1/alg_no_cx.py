...
# Actual algorithm
def ea_nn(mutation_probs, crossover_probs, sim_rum_number=0):
    lowerbound = -1
    upperbound = 1
    ...
    ''' Eventhough we register this function, cxpb is always 0, so it never fires. (NO CX) '''
    ...
    save_pickle(sts, 'no_cx/NN_nocx_sim_run_' + str(sim_rum_number), enemy, mutation_probs, crossover_probs, N_SIMS, N_POP, N_GEN)
...
if __name__ == "__main__":
    ...
    sim_rum_number = 0
    if(len(sys.argv) > 1):
        sim_rum_number = sys.argv[1]
        # mutation_probs = sys.argv[1]

    ea_nn(float(mutation_probs), float(crossover_probs), sim_rum_number)
